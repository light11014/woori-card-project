# 우리카드 소비 트렌드 분석 대시보드
우리카드 내부 직원이 고객의 분기별 소비 경향을 파악하고 카드 상품 기획 및 마케팅 전략 수립에 활용할 수 있도록,  
530만 건의 카드 거래 데이터를 8개 카테고리로 집계해 트렌드(금액/비율/증감률)를 제공하는 **내부 직원 전용 대시보드**  
운영 환경(WAS 이중화 / DB 복제 / 세션 클러스터링)을 고려한 **3-Tier Architecture** 기반으로 구현
